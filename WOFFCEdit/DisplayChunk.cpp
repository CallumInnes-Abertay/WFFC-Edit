...
DisplayChunk::DisplayChunk()
{
	...
	m_textureCoordStep = 1.0 / (TERRAINRESOLUTION - 1);
	//-1 becuase its split into chunks. not vertices.  we want tthe last one in each row to have tex coord 1
...
void DisplayChunk::RenderBatch(std::shared_ptr<DX::DeviceResources> DevResources)
{
	...
	//looping through QUADS.  so we subtrack one from the terrain array or it will try to draw a quad starting with the last vertex in each row. Which wont work
	{
		for (size_t j = 0; j < TERRAINRESOLUTION - 1; j++) //same as above
		{
			m_batch->DrawQuad(m_terrainGeometry[i][j], m_terrainGeometry[i][j + 1], m_terrainGeometry[i + 1][j + 1],
			                  m_terrainGeometry[i + 1][j]); //bottom left bottom right, top right top left.
		}
	}
	m_batch->End();
}
...
void DisplayChunk::InitialiseBatch()
{
	...
	for (size_t i = 0; i < TERRAINRESOLUTION; i++)
	{
		for (size_t j = 0; j < TERRAINRESOLUTION; j++)
		{
			...
			m_terrainGeometry[i][j].textureCoordinate = Vector2((m_textureCoordStep * j) * m_tex_diffuse_tiling,
...
void DisplayChunk::LoadHeightMap(std::shared_ptr<DX::DeviceResources> DevResources)
{
	...
	HRESULT rs;
	rs = CreateDDSTextureFromFile(device, texturewstr.c_str(), nullptr, &m_texture_diffuse);
	//load tex into Shader resource	view and resource
